...
export async function createLocalUser(username: string, passwordHash: string) { const db = await getDb(); if (!db) return undefined; await db.insert(users).values({ openId: `local:${username}`, username, passwordHash, mustChangePassword: false, name: username, loginMethod: "local", role: "user" }); return getUserByUsername(username); }
